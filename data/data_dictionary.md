...
## 포워드 추적 대상 (멤버십 정의)
- **추적 대상 = A-last 규칙(SMA200 조건부 TP50/TP100 · 365일 만기손절) 기준 현재 보유중 신호 전수.**
- 진입 유니버스 = 최근 365일 진입 전수(청산 규칙 무관). `_open_position(A)`로 현재 열림 판정 + 진입 후 365일 미만(만기 전)만 보유중. **마스터의 단순 +50%TP 시뮬레이션에 의존하지 않음**(차수·멤버십 모두 A-last 정본).
- **편입 시점 명시**(`seed_batch`): 최초 시드 = `initial`, 이후 편입분 = `membership-fix-YYYYMMDD`. 각 신호가 **언제부터 추적됐는지** 기록(포워드 정직성).
- **소급 계산 안 함**: 편입 전 공백 기간(진입일~편입일)의 경로는 **소급 산출하지 않음** — 추적은 **편입일부터** 시작. 소급하면 그것은 포워드가 아니라 **백필**임(구분 원칙).
- 청산완료·격리·추적불가(코인 미상장) 기록은 **append-only 보존**(재시드가 이력을 지우지 않음).

## 만기 미적용 버그 정정 (2026-07-13 · 투명 기재)
- **버그**: 구 seed_active가 `_open_position`에 **365일 만기손절을 적용하지 않아**, 진입 후 365일 초과(378~3937일) 포지션을 계속 "보유중"으로 오추적했다.
- **정정**: 멤버십을 A-last 정본(진입 후 365일 미만만 보유중)으로 전환하며 오추적분을 **"청산완료(백테스트)"로 재분류**(2026-07-13). 총 217건 전환.
  - **93건**: 독립 A-last 트레이드로 확정 → **day-365 시점 원장 수익률**(만기손절 92 중앙 +0.4% · TP50 1) 채움. **오늘 날짜·오늘 시세로 만기손절을 기록하지 않음**(규칙=365일 시점가, 포워드=실관측 둘 다 준수).
  - **124건**: A-last 규칙상 **이전 트레이드에 통합**(같은 티커가 다른 진입일로 계속 추적 중) → 독립 수익률 원장 없음(None). 추적은 유지되므로 손실 아님.
- **forward_closures에는 기록하지 않음** — 포워드 실관측 통계(익절률·수익종결률) 비오염. 재분류는 백테스트 청산 계열(B 집합과 동일 취급).
- 이력 삭제 없음(append-only): 구 시드 1,011 signal_id 전건 보존 + 상태만 전환.
...
- 각 지표(0–100)는 **공개 매크로 데이터(VIX·시장 국면) 기반의 검증된 진입 국면 등급** — 등급별 성과 단조성은 본 데이터셋으로 검증 가능. **산출 방법론·가중치·입력식은 비공개**(값만 제공).
...
## 체결 가정 (execution assumption)
- 산출물의 진입·청산 체결가 = **신호 확정봉의 당일 종가**(close[i]). 백테스트 엔진(`signal_exact/doha_v2.py`)이 종가 기준으로 진입·물타기·익절을 판정.
- 실 운용(Pine 알림) 규칙은 **다음 시작봉 시가**(익일 시가) 체결 → 산출물과 1봉 시차가 존재.
- 실측 괴리(무작위 80샘플, 당일종가 대비 익일시가): **진입** 중앙 +0.00%·|중앙| 1.35% · **청산** 중앙 −0.10%·|중앙| 0.81% → **소폭·무편향**(체계적 상방 왜곡 없음). 청산 평균값은 초저가(페니) 익일시가≈0 1건으로 발산하므로 중앙값이 대표.
- 해석: 표기 수익률은 **종가체결 가정**이며, 익일시가 운용 시 신호당 대략 ±1% 내외의 **무방향 슬리피지**가 추가될 수 있음(수익률의 방향성 과대계상은 아님).

## 자본 기준
- 거래당 **100만원 고정 · 단리 · 재투자 없음**. 손익 = 100만 × 수익률.
- **필터·조건별 부분집합의 성과도** 이 데이터로 직접 산출 가능(예: 품질 80+·특정 기간·시장별).
